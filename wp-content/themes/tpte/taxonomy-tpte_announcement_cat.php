<?php
/**
 * The template for displaying an Announcement category archive.
 *
 * Same layout as archive-tpte_announcement.php (download rows + Masters/categories
 * sidebar, 9 per page — see tpte_order_announcement_archive), but scoped to a single
 * announcement category. The hero shows the category name and the trail links back to
 * the main announcements archive.
 *
 * @package tpte
 */
...
$term_name     = $term ? $term->name : __( 'Ανακοινώσεις', 'tpte' );
$archive_link  = get_post_type_archive_link( 'tpte_announcement' );
?>
			...
			<div class="col-sm-12">
				<div class="tp-breadcrumb__content">
					<div class="tp-breadcrumb__list inner-after">
						...
						<span class="white"><a href="<?php echo esc_url( $archive_link ); ?>"><?php esc_html_e( 'Ανακοινώσεις', 'tpte' ); ?></a></span>
						<span class="white"><?php echo esc_html( $term_name ); ?></span>
					</div>
					<h3 class="tp-breadcrumb__title color"><?php echo esc_html( $term_name ); ?></h3>
				</div>
			</div>
		</div>
	</div>
			...
			<div class="col-lg-8">
				<div class="tp-postbox-wrapper">
					...
					if ( have_posts() ) :
						...
						?>
						<p><?php esc_html_e( 'Δεν υπάρχουν ανακοινώσεις σε αυτήν την κατηγορία.', 'tpte' ); ?></p>
						<?php
					endif;
					?>
				</div>
