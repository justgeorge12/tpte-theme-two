<?php
/**
 * Template Name: DEP Staff
 * Template Post Type: page
 *
 * Διδακτικό Ερευνητικό Προσωπικό (Δ.Ε.Π.) του Τ.Π.Τ.Ε.
		...
		<div class="container">
			<div class="row">
				...
				uasort( $dep_staff, function( $a, $b ) use ( $rank_order ) {
					$ra = isset( $rank_order[ $a['role'] ] ) ? $rank_order[ $a['role'] ] : 99;
					$rb = isset( $rank_order[ $b['role'] ] ) ? $rank_order[ $b['role'] ] : 99;
					return $ra - $rb;
				} );
				$delays = array( '.3s', '.5s', '.7s', '.9s' );
				$idx    = 0;
				foreach ( $dep_staff as $member ) :
					$delay       = $delays[ $idx % 4 ];
					$profile_url = esc_url( add_query_arg( array( 'dept' => 'dep', 'member' => $member['slug'] ), $single_url ) );
					$idx++;
				?>
